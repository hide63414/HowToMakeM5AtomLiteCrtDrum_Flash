...
- **総フレーム数の最適化とループ周期の可変制御**
  - ほぼ静止している映像は少ない枚数（30フレーム）、動きのある映像には多い枚数（165フレーム/約5.5秒分）を割り当て、限られた内蔵メモリ容量の中でアニメーションの表現力を極限まで高めています。演奏時の再生速度は一定に保ちつつ、ファイルごとにループする長さ（周期）だけが柔軟に変化します。
...
ほぼ静止している演奏パターンは少ない枚数（30F）、動きのあるパターンには多くの枚数（165F/約5.5秒分）を割り当て、再生速度は一定に保ちつつも、ファイルごとの総フレーム数に応じてループ周期を自動で切り替えるロジックを搭載しています。
...
### 💡 ノイズゲート（エフェクター）に関する補足

本システムでは、ブラウン管から拾った音を楽器として成立させるために「ノイズゲート」というエフェクターが必須の役割を担っています。

- **ノイズゲートの重要な役割**
  ブラウン管の画面に触れていないときは、スピーカーから「ブーン」という大きなハムノイズ（電源や周囲からの雑音）が常時鳴り響いてしまいます。ノイズゲートは、この不要な背景ノイズを完全にカットして無音（ミュート）にするために使用します。
  
- **音が出る仕組み**
  手がブラウン管の画面に近づいたり触れたりすると、センサーが拾う信号レベル（音量）が一気に跳ね上がります。ノイズゲートがその音量の立ち上がりを検知して瞬時にゲートを開くことで、タッチした瞬間にドラム音が出るようになります。

今回検証に使用したノイズゲートは、安価に入手可能で確実に動作が確認できているエントリーモデルです。しかし、本家の和田永さんが実際の演奏で使用されているノイズゲートなど、より上位のモデルを使用することで、さらにノイズの切れが良く、クオリティの高い音を出すことが可能です。

---
## 🌐 I2C通信による外部制御（複数台連動）
SDカードを使わなくなったことでAtomLiteのピンに空きができたため、本バージョンでは、単体でのボタン操作に加え、**I2Cバスを介した外部コントローラ機器（別のAtom Liteなど）からのリモート演奏パターン切り替え**に対応しています。  
これにより、複数台のブラウン管ドラムを一斉に、または個別に遠隔制御することが可能です。

### 1. I2C通信仕様

コントローラ機器から以下の**2バイトのデータ**を送信することで、指定したファイル番号へ即座に切り替えることができます。

*   **通信プロトコル**: I2C（ターゲット動作）
*   **制御コマンド**: `0x30` (ファイル変更コマンド)

| バイト位置 | データ内容 | 説明 |
| :--- | :--- | :--- |
| 1バイト目 | `0x30` | ファイル変更要求コマンド |
| 2バイト目 | `0 ~ fileNumberMax` | 切り替え先のファイル番号 (数値型) |

> 💡 **動作仕様**: I2C経由でファイル変更コマンドを受信した場合も、本体のボタンを長押しした際と完全に同期し、**「画像の表示切り替え」➔「変更後のファイル番号表示（400ms）」**の一連のアニメーションが自動で実行されます。  
また、変更後のファイル番号は内蔵メモリ（NVS）に自動保存されます。  
画像切り替え時にスライドイン表示を行っているため、コマンド送信間隔は十分にとってください。

---

### 2. ハードウェアによるI2Cアドレス自動設定（最大16台）

複数のブラウン管ドラムを同一のI2Cバスに接続できるよう、Atom LiteのGPIOピン（4本のデジタル入力）の状態を読み取って、自分自身のI2Cスレーブアドレスを自動的に決定する機能を搭載しています。

ベースとなるアドレスは **`0x20`** です。対象のピンはプログラム内で**内部プルダウン**に設定されているため、**何も接続しない（未配線）状態では自動的に `GND (LOW)`** になります。アドレスを変更したいピン（ビット）のみを **`3.3V (HIGH)`** に接続することで、**最大16通り（0x20 〜 0x2F）**のアドレスを個別に割り当てることができます。

##### 📌 ピン配置とアドレス加算値

| ピン名 | 役割 / ビット重み | 未接続 (LOW) | 3.3Vに接続 (HIGH) 時の加算値 |
| :--- | :--- | :--- | :--- |
| **GPIO 22** | Bit 0 (1の位) | `0` | **`+1`** |
| **GPIO 19** | Bit 1 (2の位) | `0` | **`+2`** |
| **GPIO 23** | Bit 2 (4の位) | `0` | **`+4`** |
| **GPIO 33** | Bit 3 (8の位) | `0` | **`+8`** |

##### 📋 アドレス設定の組み合わせ例

4本のピンの状態（HIGH=1, LOW=0）を4ビットの数値として読み取り、ベースアドレスにオフセットとして加算します。

*   **すべてのピンが未接続（全てLOW）**:
    オフセット `0` ➔ I2Cアドレス: **`0x20`** (1台目のデフォルト)
*   **GPIO 22（Bit 0）のみを 3.3V に接続**:
    オフセット `1` ➔ I2Cアドレス: **`0x21`**
*   **GPIO 19（Bit 1）と GPIO 33（Bit 3）を 3.3V に接続**:
    オフセット `2 + 8 = 10 (0x0A)` ➔ I2Cアドレス: **`0x2A`**
*   **すべてのピンを 3.3V に接続（全てHIGH）**:
    オフセット `1 + 2 + 4 + 8 = 15 (0x0F)` ➔ I2Cアドレス: **`0x2F`**

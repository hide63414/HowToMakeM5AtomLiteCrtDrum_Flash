...
複数のブラウン管ドラムを同一のI2Cバスに接続できるよう、Atom LiteのGPIOピン（4本のデジタル入力）の状態を読み取って、自分自身のI2Cアドレスを自動的に決定する機能を搭載しています。

ベースとなるアドレス `0x20` に対し、4つのピンを `3.3V (HIGH)` にプルアップ（接続）するか、`GND (LOW)` にプルダウンするかによって、**最大16通り（0x20 〜 0x2F）**のアドレスを個別に割り当てることができます。

#### 📌 ピン配置（アドレス設定用）

| ピン名 | 役割 / ビット重み | HIGH (3.3V) 時の加算値 |
| :--- | :--- | :--- |
| **GPIO 22** | Bit 0 (1の位) | `+1` |
| **GPIO 19** | Bit 1 (2の位) | `+2` |
| **GPIO 23** | Bit 2 (4の位) | `+4` |
...
4本のピンの状態をバイナリ（4ビット）として読み取り、ベースアドレスにオフセットとして加算します。

*   **すべてのピンが未接続 (全てLOW)**:
    オフセット `0` ➔ I2Cアドレス: **`0x20`**
*   **GPIO 22（Bit 0）のみを3.3Vに接続**:
    オフセット `1` ➔ I2Cアドレス: **`0x21`**
*   **GPIO 19（Bit 1）と GPIO 33（Bit 3）を3.3Vに接続**:
    オフセット `2 + 8 = 10 (0x0A)` ➔ I2Cアドレス: **`0x2A`**
*   **すべてのピンを3.3Vに接続 (全てHIGH)**:
